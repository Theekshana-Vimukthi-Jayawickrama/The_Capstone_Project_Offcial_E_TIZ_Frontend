...
const adminId = sessionStorage.getItem('adminId')
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/v1/adminRoute/routeDeleteList');
...
const filteredItems = items.filter(item => {
  const cleanedTitle = item.districtName.toLowerCase().replace(/[^\w\s]/gi, ''); // Remove symbols
  ...
  return (
    <>
      ...
      <div className='container'><NavBar /></div>
       <center><h1 className="no-focus-outline">Delete Bus Routes' List</h1></center>
              ...
              <div className='col-5 p-4 mx-4 my-2 zoom-opacity-effect' key={item.id} >                                                                           
                  <div className='row border'>
                    <div className='col-12  mr-4 text-light bg-dark'>
                      <h2 className='text-center fs-3  m-2 ' >{item.route}</h2>
...
export default RouteDeleteDetails;
