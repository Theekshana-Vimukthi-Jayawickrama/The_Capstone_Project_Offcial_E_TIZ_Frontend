...
const ConductorPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:8080/api/v1/admin/getConductorShortDetails`, {
                    headers: {
    ...
    const editPost = (itemId) => {
        navigate(`/editConductorPage?id=${itemId}`);
    };
  ...
  const textStyle = {
    ...
    return (
        <>
           ...
           <div className='fs-4 text-center'><b>CONDUCTORS</b></div>
            <div className='container'>
                <div className=' my-4 row '>
                    <div className='col-lg-8'>
                    <div className='input-group mb-3'>
                        <label htmlFor="search" className='d-none d-sm-block input-group-append mx-3' style={textStyle}>Search by ID: </label>
                        <input
                    ...
                    <button className="btn m-2 my-sm-0 btn-outline-secondary mr-4" type="button" onClick={handleAddConductors} style={textStyle}>
                            Add New Conductor
                    </button>
                </div> 
                </div>
                ...
                {currentItems.length !== 0 ? currentItems.map(item => (
                    <div className='container-fluid border m-4 border-3 border-secondary' key={item.id}>
                            ...
                            <div className='row my-2 font-weight-bold text-dark'>    
                                <div className='col-lg-10'><Link to={`/conductorDetails/?id=${item.id}`} className=" text-decoration-none">
                                    
                                    <div className='row '>
                                        <div className='col-lg-2 col-md-4 m-2'>
                                            <center><img
                                        ...
                                        <div className='col-lg-2 col-md-6  m-3 font-weight-bold text-dark'>
                                                <div className="fs-5 text-center">Conductor ID: {item.userName}</div>                                    
                                        </div>                                         
                                    </div>        
